...
```c
sizeof()返回的结果是size_t类型, 是无符号整型, 如果为负数,那么它会被认为是一个巨大的正数
```
...
| %f   | 打印float类型的数据                                          |
| %lf  | 打印double类型的数据                                         |
| %zu  | 打印sizeof的返回值 , sizeof()返回的结果是size_t类型, 是无符号整型, 如果为负数,那么它会被认为是一个巨大的正数 |
...
作为无符号位看的时候, 所有位都是有效位, 为42亿多
在计算的时候,有符号和无符号, 会把有符号转换为无符号数, 如果类型不同还会进行算术转换 (整型提升)
